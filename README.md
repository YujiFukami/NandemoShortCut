...
- `設定読み込み` から別の `config.json` を読み込めます。読み込み前に現在設定のバックアップを作成します
- `設定書き出し` から現在の設定を任意の場所へ保存できます
- `設定フォルダ` から現在使っている設定ファイルのフォルダを開けます
- `使い方`、`開発元`、`GitHub` ボタンから関連ページを開けます
- アクション一覧の `内容` 列で、コピー文字列、URL、ファイルパス、フォルダ名などを確認できます
- `テキストコピー` のコピー内容は複数行で編集できます
- `最新画像コピー` で、指定した画像フォルダ内の最新画像ファイルをコピー待機状態にできます
